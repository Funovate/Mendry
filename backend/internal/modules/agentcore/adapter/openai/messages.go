...
// messagesJSONInstruction 在 ResponseFormatJSONObject 下替代 Anthropic 缺失的 json_object 模式。
...
func buildMessagesRequest(model string, maxTokens int, messages []chatMessage, tools []chatTool, format ResponseFormat) (messagesRequest, error) {
	...
	for _, message := range messages {
		...
		case "assistant":
			...
			for _, call := range message.ToolCalls {
				...
				if !json.Valid(input) {
					return messagesRequest{}, errors.New("encode anthropic messages input: invalid tool arguments")
				}
				blocks = append(blocks, messagesContentBlock{Type: "tool_use", ID: call.ID, Name: call.Function.Name, Input: input})
			}
			appendBlocks("assistant", blocks...)
		default:
			return messagesRequest{}, errors.New("encode anthropic messages input: invalid role")
		}
	}
...
func decodeMessagesResponse(body []byte, format ResponseFormat) (chatResponse, error) {
	...
	if len(choice.Message.ToolCalls) == 0 {
		...
		if format == ResponseFormatJSONObject {
			choice.Message.Content = extractJSONObject(choice.Message.Content)
		}
	}
	...
	return chatResponse{Choices: []chatChoice{choice}, Usage: chatUsage{
		...
		PromptCacheHit: &hit, PromptCacheMiss: &miss,
	}}, nil
}

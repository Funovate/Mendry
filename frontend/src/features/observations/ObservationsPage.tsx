...
export function ObservationsPage() {
  ...
  return <section className="settings-view"><div className="view-header"><div><div className="eyebrow">{project.name}</div><h1>Event stream</h1><p>Persisted observations from this project, ordered newest first by the API.</p></div></div><div className="event-list">{observations.data.items.map((item) => <article key={item.id}><span className={`level ${item.level.toLowerCase() === "warn" ? "warn" : ""}`}>{item.level}</span><code><time>{formatTime(item.occurredAt)}</time> {item.message}</code><span>{item.service || item.sourceId}</span></article>)}{observations.data.items.length === 0 && <div className="table-empty">No observations have been collected for this project.</div>}</div></section>;
}

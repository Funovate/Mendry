...
### Mistake 2b: Omitted Versus Empty Write-Only Fields

**Bad**: A Git edit form that collected only a username composes `username:` and
then omits `value`, so the backend treats the request as name-only and keeps the
old ciphertext.

**Good**: HTTP `nil` means omit. An explicit empty string is invalid. Incomplete
replacement drafts stay incomplete in the UI until the encoded value is whole.
See `.trellis/spec/backend/project-guidelines.md` and

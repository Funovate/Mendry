...
项目拥有自己的成员、配置、凭据、观测记录、事故、修复记录和审计事件。
...
创建者获得项目 `admin` 权限。系统角色与项目角色彼此独立。
...
首次保存 `signed_webhook` 时会生成随机入站 token。只有项目管理员能读取或轮换完整入站 URL。请设置 `FIXTHE_PUBLIC_URL`，使 API 能生成该 URL。

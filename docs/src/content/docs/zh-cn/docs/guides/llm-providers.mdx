...
2. 保存 provider `openai`、兼容 HTTPS 的 `baseUrl`、credential ID、model ID 与 API 模式：
...
已有 provider 记录默认使用 `chat_completions`。连接 probe、日志规则生成和 remediation model turn 会一致使用所选模式；两个模式共享 `/v1/models` 模型发现调用。
